...
public class Code {
	public static void main(String[] args) {
		...
		System.out.println("Wlcome to Guess the Number Game!");
		...
		while(currentRound <= totalRounds) {
			...
			while(true){
				...
				if(attempts >= maxAttempts) {
					System.out.println("Sorry, you have reaches the maximum number of attempts. The correct number was: " + targetNumber);
					break;
				}
	...
	private static int calculateRoundScore( int attempts) {
		...
		// Fell free to customize this method as per your scoring rules.
		if(attempts <= 3) {
			return 10;
		}else if (attempts <= 6) {
			return 5;
